...
def pivots_points(dataframe: pd.DataFrame, timeperiod=10, levels=3) -> pd.DataFrame:
    """
    Pivots Points
    Formula:
...
class CactiX(IStrategy):
    ...
    def populate_buy_trend(self, dataframe: DataFrame) -> DataFrame:
        ...
        dataframe.loc[
           (
               (
                   (qtpylib.crossed_above(dataframe['close'], dataframe['r1'])) |
                   (qtpylib.crossed_above(dataframe['close'], dataframe['s1']))
               )
    ...
    def populate_sell_trend(self, dataframe: DataFrame) -> DataFrame:
        ...
        dataframe.loc[
           (
               (
                   (qtpylib.crossed_above(dataframe['r1'], dataframe['close'])) |
                   (qtpylib.crossed_above(dataframe['s1'], dataframe['close']))
               )
